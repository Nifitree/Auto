...
def run_goodsPM_3_custom():
    step_name = "goodsPM_3 (Custom Flow)"
    app = None
    ...
    try:
        ...
        SERVICE_TITLE = S_CFG['GOODSPM_3_TITLE']        # 50418
        ...
        # 3. กรอกบาร์โค้ด
        barcode_val = S_CFG.get('PM3_BARCODE', '|0105539106911000000000060000092660')
        # AutoID: Barcode_50418
        fill_field_by_id(main_window, "Barcode_50418", barcode_val, "บาร์โค้ด")

        # 4. กดถัดไป (เพื่อไปหน้ากรอกชื่อ)
        press_next_button(main_window)

        # 5. กรอกข้อมูลส่วนที่เหลือ
        
        # 5.1 REFNO4: ชื่อ-สกุล
        name_val = S_CFG.get('PM3_NAME', 'Test Name')
        fill_field_by_id(main_window, "REFNO4", name_val, "ชื่อ-สกุล")

        # 5.2 Amount: จำนวนเงิน
        amt_val = S_CFG.get('PM3_AMOUNT', '100.00')
