...
if __name__ == "__main__":
    ...
    try:
        ...
        # 2. พิมพ์บาร์โค้ด (Barcode_51120)
        BARCODE_ID = S_CFG['BARCODE_INPUT_ID5']
        BARCODE_VAL = S_CFG.get('P5_BARCODE', '')
        
        print(f"[*] Typing Barcode: {BARCODE_VAL}")
        main_window.child_window(auto_id=BARCODE_ID).click_input()
        time.sleep(0.3)
        main_window.child_window(auto_id=BARCODE_ID).type_keys(BARCODE_VAL, with_spaces=True)
        time.sleep(1)
