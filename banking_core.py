...
def connect_main_window():
    """เชื่อมต่อหน้าต่างหลักผ่าน AppContext"""
    return ctx.connect()
...
def banking_services_main():
    """Logic เข้าเมนู A -> B -> อ่านบัตร -> กรอกข้อมูล"""
    try:
        app, main_window = connect_main_window()
        print(f"[*] Navigating to Banking Main ({B_CFG['HOTKEY_AGENCY_TITLE']} -> {B_CFG['HOTKEY_BaS_TITLE']})...")
...
def run_banking_transaction(main_window, title, double_next=False):
    """ทำรายการย่อย (เลือก -> ถัดไป -> [ถัดไป] -> เสร็จสิ้น)"""
    ...
    if double_next:
        print("[*] Clicking Next (2) [Double Next Logic]...")
        main_window.child_window(title=B_CFG["NEXT_TITLE"], auto_id=B_CFG["ID_AUTO_ID"], control_type="Text").click_input()
        time.sleep(WAIT_TIME)
...
def run_service(step_name, service_title, use_main=True, use_search=False, double_next=False):
    ...
    try:
        ...
        # เชื่อมต่อ App (ผ่าน ctx)
        app, main_window = connect_main_window()

        # นำทางเข้าหน้าหลัก (ถ้าจำเป็น)
        ...
        if app: save_evidence_context(app, {"test_name": "Banking", "step_name": step_name, "error_message": str(e)})
        print(f"[X] FAILED: {e}")